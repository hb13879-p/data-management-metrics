...
def index():
    ...
    return render_template('homepage.html', form=form, db_selected='f', csv_selected='f')
...
def profiler_result():
    ...
    return render_template('calculating.html', connector=connector, db_input_file=db_input_file,db_schema=schema)
...
def output():
    ...
    dashboard = StandardDashboard(
                        ...
                        ,[TotalBlankCells(bcm_data_source,{'pc':True}), TotalRowsCols(bcm_data_source), DuplicateRows(bcm_data_source), ExtractPIIAttributes(bcm_data_source)]
                        ,ClassifyClientNotes(bcm_data_source, {'id_col':'user_id','notes_col':'client_notes'})
                        )
    dashboard.calculate_dashboard()
    cols = dashboard.get_column_names()
    headline_metric_results, headline_metric_labels, headline_metric_icons, headline_metric_colours = dashboard.get_headline_metrics()
    session['dashboard'] = dashboard
    return render_template('dashboard.html',no_headline_metrics=len(headline_metric_results)
...
def get_tabular_data():
    ...
    return jsonify(tabular=df.to_html(table_id='tabular_table',classes=['compact', 'stripe','small','responsive'],index=False))      
...
def get_targets_data():
    ...
    return jsonify(targets=df.to_html(table_id='targets_table',classes=['compact', 'stripe','small','responsive'],index=False))
...
def get_anomaly_data():
    ...
    df = df.head(10)
    return jsonify(anom_data=df.to_html(table_id='anomaly_table',classes=['compact', 'stripe','small','responsive'],index=False))      
...
def get_ml_address_data():
    ...
    df = df.drop_duplicates(subset=['addr','Validity Score']).head(20)#.sort_values('Validity Score')
    return jsonify(ml_addr_data=df.to_html(table_id='ml_addr_table',classes=['compact', 'stripe','small','responsive'],index=False))      
...
def get_postcode_data():
    ...
    df = df.head(10)
    return jsonify(postcode_data=df.to_html(table_id='postcode_table',classes=['compact', 'stripe','small','responsive'],index=False))      

@app.route('/get_data_rules_data', methods=['GET','POST'])
def get_data_rules_data():
    dashboard = session['dashboard']
    data_rules, ai_rules = dashboard.get_data_rules_view()
    return jsonify(data_rules_data=data_rules,
                ai_rules_data=ai_rules)      
...
def get_client_notes_data():
    ...
    df = df.drop_duplicates(subset=['client_notes','Confidence Score'])#.sort_values('Confidence Score', ascending=False)
    app.logger.debug(df)
    return jsonify(client_notes_data=df.to_html(table_id='client_notes_table',classes=['compact', 'stripe','small','responsive'],index=False))