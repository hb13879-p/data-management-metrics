...
class Metric(object):
    ...
    def get_result(self):
        ...
        if self.result is None:
            raise ValueError('You need to calculate the metric value before you can return the result. Call the metric at least once before calling get_result()')
        else:
            return self.result
    ...
    def __call__(self, **kwargs) -> pd.DataFrame:
        ...
        if isinstance(self.data_source,InMemoryDataSource):
            self.result = self.data_source.run_metric(self.calculate_in_mem,**metric_args)
        elif isinstance(self.data_source,SQLDataSource):
            self.result = self.data_source.run_metric(self.calculate_sql_tbl,**metric_args)
        return self.result
    ...
    def calculate_in_mem(inp: pd.DataFrame, incl_graph=True) -> pd.DataFrame:
        def graph_select(row, inp):
            if row['02. Data Type'] == 'object' and row['06. No. Unique Values'] > 50:
                bar_chart_df = row[['04. Nulls', '05. Non-Nulls', '03. Row Count', '06. No. Unique Values']]
                bar_chart_data = pd.DataFrame(data=bar_chart_df.astype('str').values,columns=['value'])
                bar_chart_data['col_names'] = ['Null Values', 'Total Non-Null Values', 'Total Values inc. Nulls', 'Distinct Values']
                fig = px.bar(bar_chart_data,x='col_names',y='value')
                return json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
            else:
                return json.dumps(px.histogram(inp, x=row['01. Column Name'], nbins=75), cls=plotly.utils.PlotlyJSONEncoder)
        ...
        try:
            ...
            profile = profile.merge(stats.T,how='left',left_on='01. Column Name',right_index=True)
            profile = profile.rename(columns={"mean": "07. Average Value", 
...
#            profile['histogram_json'] = profile['01. Column Name'].apply(lambda col: json.dumps(px.histogram(inp, x=col, nbins=20), cls=plotly.utils.PlotlyJSONEncoder))
            profile['histogram_json'] = profile.apply(lambda x: graph_select(x,inp),axis=1)
    ...
    def calculate_sql_tbl(sql_view_connector: SQLViewConnector) -> pd.DataFrame:
        if sql_view_connector.table_name is not None: # this will be None if custom sql option is chosen 
            column_info_query = text('''select * from  information_schema.columns where table_schema = :db_name and table_name = :table''')
            column_info_params = {'db_name': sql_view_connector.sql_connector.db_creds['dbname'], 'table': sql_view_connector.table_name}
            info_schema_results = sql_view_connector.sql_connector.run_query(column_info_query,column_info_params)
    ...
    def get_result_for_column(self, col_name, round_dec=2):
        ...
        print(result[result.select_dtypes(include=['number']).columns])
        result[result.select_dtypes(include=['number']).columns] = result.select_dtypes(include=['number']).round(round_dec)
        print(result[result.select_dtypes(include=['number']).columns])
        result = result.loc[result['01. Column Name']==col_name].dropna(axis=1).to_json(orient='records')        
        return result
    ...
    def get_targets_view(self, round_dec=2):
        ...
        df['completeness_pc'] = np.round(np.random.randint(91,100,size=(20, 1)) + np.random.rand(20,1),2)
        df['validity_pc'] = np.round(np.random.randint(91,100,size=(20, 1)) + np.random.rand(20,1),2)
        df['bespoke_tests_pc'] = np.round(np.random.randint(91,100,size=(20, 1)) + np.random.rand(20,1),2)
        return df
...
class TotalBlankCells(Metric):
    ...
    def calculate_sql_tbl(sql_view_connector: SQLViewConnector, pc: bool = True):
        def create_tot_blank_cell_query(column_names: List[str], db_name: str, tbl_name: str) -> str:
            query = 'select '
            for col in column_names:
                q = 'sum(case when ' + col + ' is null or ' + col + ' = \'\' then 1 else 0 end) +'
                query += q
        ...
        if sql_view_connector.table_name is not None: # this will be None if custom sql option is chosen 
            query = create_tot_blank_cell_query(sql_view_connector.column_names,sql_view_connector.sql_connector.db_creds['dbname'], sql_view_connector.table_name)
...
class TotalRowsCols(Metric):
    ...
    def calculate_sql_tbl(sql_view_connector: SQLViewConnector) -> Tuple[int,int]:
        if sql_view_connector.table_name is not None: # this will be None if custom sql option is chosen 
            query = text('''select count(*) from ''' + sql_view_connector.sql_connector.db_creds['dbname'] + '.' + sql_view_connector.table_name)
            results = sql_view_connector.sql_connector.run_query(query)
    ...
    def calculate_sql_tbl(sql_view_connector: SQLViewConnector) -> np.int64:
        def create_duplicate_rows_query(column_names, db_name, table_name):
            ...
            for col in column_names:
                query += col + ','
                groupby += col + ','
            query = query[:-1] + ' from ' + db_name + '.' + table_name + groupby[:-1] + ' having count(*) > 1) as t'
            return text(query)

        if sql_view_connector.table_name is not None: # this will be None if custom sql option is chosen 
            query = create_duplicate_rows_query(sql_view_connector.column_names, sql_view_connector.sql_connector.db_creds['dbname'], sql_view_connector.table_name)
            results = sql_view_connector.sql_connector.run_query(query)
...
class DetectBadAddress(Metric):
    ...
    @staticmethod
    def calculate_in_mem(inp: pd.DataFrame, id_col: str = 'id',address_col: List[str] = ['addr','city'], model_path: str = r'C:/Users/hbinning/Documents/data_profiling_tool/app/models/bow_model_pipeline_v2.joblib') -> pd.DataFrame:
        '''
        ...
        addr['addr'] = addr['addr'].apply(lambda x: re.sub(r'[^\x00-\x7F]+','', str(x)))
...
class ClassifyClientNotes(Metric):
    ...
    @staticmethod
    def calculate_in_mem(inp: pd.DataFrame, id_col: str = 'id',notes_col: str = 'client_notes', model_path: str = r'C:/Users/hbinning/Documents/data_profiling_tool/app/models/notification_model.joblib') -> pd.DataFrame:
        '''
        ...
        df = df[[id_col,notes_col,'Confidence Score']]#.loc[~addr['addr'].str.isupper()]
...
class ExtractDataRules(Metric):
    ...
    @staticmethod
    def calculate_in_mem(inp: pd.DataFrame) -> pd.DataFrame:
        rules = ['"user_id" is unique', '"Email_Address" is unique','"product_start_date" contains 94.9% null values', '"dob" can be transformed to exactly match "cleaned_dob" with the following transformation: "Remove elements 2 & 5"','"gender" takes the values ("M","F")', '"dob" is between "01/01/1950" and "31/12/2001"']
        ai_rules = ['Rows with "Student Credit Card" are 81.5% likely to have "Interest-only Mortgage"','Rows with "Fixed-rate Mortgage" are 74.3% likely to have "Classic Credit Card"','"ENGLAND" and "UNITED KINGDOM" always imply "GB" - storage space could be saved by normalising']
...
class ExtractBadPostcode(Metric):
    ...
    @staticmethod
    def calculate_in_mem(inp: pd.DataFrame, id_col: str = 'id', postcd_col: str = 'postcode') -> pd.DataFrame:
        '''
        ...
        Returns:
            pd.DataFrame:ID Column,    
        '''
        uk_postcode = re.compile('([Gg][Ii][Rr] 0[Aa]{2})|((([A-Za-z][0-9]{1,2})|(([A-Za-z][A-Ha-hJ-Yj-y][0-9]{1,2})|(([A-Za-z][0-9][A-Za-z])|([A-Za-z][A-Ha-hJ-Yj-y][0-9][A-Za-z]?))))\s?[0-9][A-Za-z]{2})|([0-9][0-9][0-9][0-9][0-9])')
...
class GroupedZScore(Metric):
    ...
    @staticmethod
    def calculate_in_mem(inp: pd.DataFrame, id_col: str = 'id', group_key: str = 'card_type', group_value: str = 'credit_rate', conf: float = 0.99) -> pd.DataFrame:
        '''
        Returns bad postcodes from a list of postcodes.
        ...
        df[group_key] = df[group_key].apply(lambda x: re.sub(r'[^\x00-\x7F]+','', str(x)))
...
class SupervisedAnomalyDetection(Metric):
    ...
    @staticmethod
    def calculate_in_mem(inp: pd.DataFrame, id_col: str = 'id', x_cols: List[str] = ['input'], y_col: str = 'output', model_path: str = r'models/premium_model.joblib') -> pd.DataFrame:
        '''
