...
class SQLConnector(object):
    ...
    def __init__(self, db_creds: Dict[str,str]):
        if self.prefix is None:
            raise ValueError('Please instatiate subclass by choosing a sql dialect and using factory method eg SQLConnector.sql_connector_factory("mysql", db_creds)')
    ...
    def create_conn_string(self, prefix: str, db_creds: Dict[str,str]):
        return '{0}://{1}:{2}@{3}:{4}/{5}?charset=utf8mb4'.format(
                                                                prefix, db_creds['user'], db_creds['password'], db_creds['server'], str(db_creds['port']), db_creds['dbname']) 
...
class SQLViewConnector(object):
    ...
    def __init__(self, sql_dialect: str, db_creds: Dict[str,str], table_name: str = None, custom_sql: str = None, custom_sql_params: str = None):
        ...
        if table_name is not None and custom_sql is not None:
            ...
            raise ValueError('Please provide either a table name or custom SQL returning a table')
        if table_name is not None:
            self.main_query_sql = text('select * from ' + db_creds['dbname'] + '.' + table_name) 
        ...
        self.column_names = self.sql_connector.get_query_metadata(self.main_query_sql, params=self.main_query_params)
